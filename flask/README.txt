...
triPOD Installation and Usage Instructions
...
Required perl and R libraries - The following perl modules need to be
	installed: Algorithm::Cluster, threads, and Tree::Interval.
	The following R libraries need to be installed: library(shape)
	and library(TTR).

Extraction - Extract the files. You should now have a directory named 
	/triPOD_current, which contains the files necessary to run triPOD. 
	Be sure to keep these files in a single directory. 

Run triPOD - triPOD is run by executing the Perl script, which makes
	use of the R script and the genome build files.  Example :
	perl triPOD.pl your_data.txt

Help - triPOD.pl --help The help function provides the following
	information regarding optional parameters and expected input
	formatting

	Usage: perl triPOD.pl [--options] [INPUT_FILE]

	--alpha 	A threshold for which a p-value <= alpha is considered
			significant.  Default = --alpha=0.1
	...
	--graph 	Creates graphic output of results in PNG or PDF format
			(--graph=none, --graph=png, --graph=pdf, or --graph=both)
            		Default = --graph=none
	--hd 		Abnormality detection by homozygous deletion analysis
			(PODhd) (--hd or --nohd) Default = --hd
	...
	--homSD 	Homozygous SNP threshold. StDevs from mean BAF 
			Default = --homSD=4
	--mi1 		Abnormality detection by Mendelian "error" analysis
			(PODmi1) (--mi1 or --nomi1) Default = --mi1
	--nc 		Samples must have a No Call rate below this threshold 
			to be analyzed.  Default = --nc=0.03
	...
	--stats 	Create a file including calculated parameters, etc.
			(--stats) Not default.
	--verbose 	Prints progress info to screen. Negated using
			--noverbose. --Default = --verbose.	
			--batch mode will run in --noverbose mode.
	--win 		Number of SNPs per window for sliding window analysis
			Default = --win=100

Note -  It is highly recommended that parameters such as --alpha, --hetSD,
       --homSD, and --win are altered only be experienced users.

Input - The input file must be tab delimited, sorted by chromosome and
	position, and in the following order (columns): SNP Name, Chromosome,
	Position, Father GType, Father BAF, Father LRR, Mother GType,
	Mother BAF, Mother LRR, Child GType, Child BAF, Child LRR.
	The genotypes must be AA, AB, BB, NC or NoCall.  B allele
	frequencies must be >= 0 and <= 1 for polymorphic markers.  A
	header line is expected but is not used to determine column
	identity.

	triPOD has been developed using genotypes annotated by the 
	Illumina method described in "TOP/BOT" Strand and "A/B" Allele.
	Illumina Technical Note. 2006. If converting from HapMap format
	(ATCG) to Illumina format (AB): use simple replacement (AA = AA, TT; 
	BB = CC, GG; AB = AC, AG, TC, TG; -- = NC). Any markers with 
	alternative genotyping combinations (e.g. CG or TA) should be 
	discarded unless the user performs an additional analysis of 
	the surrounding sequence.

Output - Several types of output are produced by triPOD.
	1. *triPOD_Results.txt A tabular listing of detected abnormalities
	...
	Method, Descriptive statistics for the abnormal region: 
	...
	The results file also provides the date and time, the command
	line information, and the parameters employed.
	
	2. *triPOD_Results.bed 
	...
	parameters.  See the paper for explanations of rate and
	probability calculations. The following information is
	included for each trio: file name, trio member names, NoCall
	rates, nonadjacent homozygous deletion (HD) and single
